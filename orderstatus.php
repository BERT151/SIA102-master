...
include'include/header.php';
    ...
    $tuser_products_query="select * from transaction where Account_ID='$Account_ID' Order by Date DESC";
    $tuser_products_result=mysqli_query($con,$tuser_products_query) or die(mysqli_error($con));
    $tno_of_user_products= mysqli_num_rows($tuser_products_result);
    if($tno_of_user_products==0){ 
  
    }else{
    while($row=mysqli_fetch_array($tuser_products_result)){
    ...
    $user_products_query="select * from transaction ut inner join products it on it.id=ut.Product_ID where ut.Account_ID='$Account_ID' AND Order_ID= '$orderid' GROUP BY ut.Order_ID ";
    ...
    while($row=mysqli_fetch_array($user_products_result)){
     ...
     $Account_ID=$_SESSION['id'];
    $buser_products_query="select * from transaction where Account_ID='$Account_ID' GROUP BY Order_ID";
